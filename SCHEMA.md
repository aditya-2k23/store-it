...
| 2026-07 | v1.9    | Hierarchy fixes — improved `move_folder` with `FOR UPDATE` locking to prevent deadlocks and used `coalesce` for nullable paths. `cascade_trash_folder` now preserves timestamps for items already trashed, and `cascade_restore_folder` securely checks `trashed_at` to avoid reviving independently trashed items and ancestors.                                    |
